...
bool enable_coll_avoidance = true;
...
// Update the cur_move struct data.
void xy_fsm(void) {
	...
	int new_fwd = sqrt(dx * dx + dy * dy);   //The distance need to move, unit is mm.
	int new_ang = atan2(dy, dx) * RAD2nDEG; //[-pi,+pi], 1 nDeg = 1x10^-9 Deg

	if (back_mode_store == 1) { // Force backwards
		new_fwd *= -1;
		new_ang = (uint32_t) new_ang + ANG_180_DEG;
	} else if (back_mode_store == 2) { // Auto decision
		int ang_err = cur_pos.ang - new_ang;
		if ((ang_err < (-0.75 * ANG_180_DEG) || ang_err > (0.75 * ANG_180_DEG)) && new_fwd < 1000) { // 0.75*180deg
			new_fwd *= -1;
			new_ang = (uint32_t) new_ang + ANG_180_DEG;
		}
	}
	...
	if ((ang_changed < -90 * ANG_1_DEG || ang_changed > 90 * ANG_1_DEG)
		...
		store_stop_flags |= 1UL << 11;	//stop
		reset_movement();
		cur_move.state = MOVE_IDLE;
	}

	cur_move.rel_fwd = new_fwd; //Next step should move.
...
void move_xy_abs(int32_t x, int32_t y, int back_mode, int id, int speedlim) {
	...
	int new_ang = atan2(dy, dx) * RAD2nDEG; //(4294967296.0 / (2.0 * M_PI));
	...
	if (back_mode == 1) {  // Force backwards
		...
		int ang_err = cur_pos.ang - new_ang;
		if ((ang_err < ANG_DEG(-135) || ang_err > ANG_DEG(135)) && new_fwd < 1000) {  // 0.75*180deg
			new_fwd *= -1;
			new_ang = (uint32_t) new_ang + ANG_180_DEG;
		}
	}
...
// Update the cur_move struct data, and do speed skew.
void navig_fsm1(void) {
	dbg_teleportation_bug(211);

	xy_fsm();  // Update the cur_move struct data.
	//speed skew limit
...
void daiju_meininki_fsm(void) {
	...
	if (lidar_collision_avoidance_new) {
		...
		for (int i = (360 - 60); i < 360; i += 2) {
			if (!lidar_collision_avoidance[i].valid) {
				continue;
			}

			int dist_to_front = lidar_collision_avoidance[i].x - robot_origin_to_front;

			if (lidar_collision_avoidance[i].y > -1 * (robot_ys / 2 + 10)
					&& lidar_collision_avoidance[i].y < (robot_ys / 2 + 10)) {
				if (dist_to_front < nearest_colliding_front) {
					nearest_colliding_front = dist_to_front;
				}
			}
		}
		...
		for (int i = 180 - 60; i < 180 + 60; i += 2) {
			...
			if (lidar_collision_avoidance[i].y > -1 * (robot_ys / 2 + 0)
					&& lidar_collision_avoidance[i].y < (robot_ys / 2 + 0)) {
				if (dist_to_back < nearest_colliding_back) {
					nearest_colliding_back = dist_to_back;
				}
			}
		}
		...
		for (int i = 180 + 35; i < 180 + 90 + 1; i += 2) {
			...
			if (lidar_collision_avoidance[i].y > -1 * (robot_ys / 2 + 40)) {
				if (lidar_collision_avoidance[i].x < 0
						&& lidar_collision_avoidance[i].x > -(robot_origin_to_back + 10)) {
					can_do[TURN_RIGHT] = 0;
					break;
				}
...
*/
	if (correcting_angle()) {
		...
		if (ang_remain > ANG_DEG(5) || ang_remain < ANG_DEG(-5)) {
			int hit_remain_mm;
			if (ang_remain > 0) {
...
#define CHAFIND_PASS1_ACCEPT_ANGLE (ANG_DEG(2)) // was 1 deg
...
void chafind_empty_accum1(void) {
	chafind_nearest_hit_x = 9999;
	chafind_nearest_hit_y = 0;
}

void chafind_empty_accum2(void) {
...
//		dbg[] = chafind_left_accum_cnt;
	...
	} else if (y > (chafind_nearest_hit_y + CHAFIND_LOOK_SIDEWAY_MIN)
			&& y < (chafind_nearest_hit_y + CHAFIND_LOOK_SIDEWAY_MAX) &&
//		x > CHAFIND_FIRST_DIST-CHAFIND_FIRST_TOLERANCE-100+CHAFIND_MIDDLE_BAR_DEPTH && x < CHAFIND_FIRST_DIST+CHAFIND_FIRST_TOLERANCE+100+CHAFIND_MIDDLE_BAR_DEPTH)
			x > 100 && x < 700) {
...
//		dbg[] = chafind_right_accum_cnt;
	...
	if (x >= chafind_nearest_hit_x && x < chafind_nearest_hit_x + 60
			&& y > chafind_nearest_hit_y - CHAFIND_MIDDLE_BAR_WIDTH - 15
			&& y < chafind_nearest_hit_y + CHAFIND_MIDDLE_BAR_WIDTH + 15) {
		chafind_middle_cnt++;
		if (y < chafind_middle_min_y) {
			chafind_middle_min_y = y;
		}
		if (y > chafind_middle_max_y) {
			chafind_middle_max_y = y;
		}
...
void navig_fsm2_for_charger(void) {
	static int pass;
	static int timer;
	...
	switch (chafind_state) {
		...
		case CHAFIND_ACCUM_DATA: {
			if ((pass == 0 && chafind_left_accum_cnt > 30 && chafind_right_accum_cnt > 30 && chafind_middle_cnt > 20)
					|| (pass > 0 && chafind_left_accum_cnt > 60 && chafind_right_accum_cnt > 60
							&& chafind_middle_cnt > 40)) {
				pass++;
				int ang, shift, dist;
				...
				if (ang > -1 * CHAFIND_PASS1_ACCEPT_ANGLE && ang < CHAFIND_PASS1_ACCEPT_ANGLE
					...
					if (shift > -1 * CHAFIND_PASS1_ACCEPT_SHIFT && shift < CHAFIND_PASS1_ACCEPT_SHIFT) { // Only turning needed
						...
						int shift_ang;
						if (shift > 170) {
							shift_ang = ANG_DEG(30);
						} else if (shift > 70) {
							shift_ang = ANG_DEG(15);
						} else if (shift > 0) {
							shift_ang = ANG_DEG(8);
						} else if (shift < -170) {
							shift_ang = ANG_DEG(-30);
...
// Processing the move mode.
void navig_fsm2(void) {
	if (bat_emerg_action) {	//action for battery emergency event.
		stop_navig_fsms();
		stop_movement();
		host_alive_long();
		set_top_speed(15);
	...
	if (chafind_state) {	//Find Charger mode.
		lidar_near_filter_on = false;
		lidar_midlier_filter_on = false;

		navig_fsm2_for_charger();
		return;
	} else {
		lidar_near_filter_on = true;
		lidar_midlier_filter_on = true;
	}
